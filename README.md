...
DTM制作者向けのリアルタイムボーカルピッチトレーナーWebアプリ。ブラウザ上で動作し、リアルタイム共同編集にも対応。
...
- **ルーム管理**: 最大2人のユーザーが招待リンクで接続
  - ルーム作成 → URLベースの招待リンク共有 → 参加時に全状態スナップショット同期
...
- **リモートカーソル**: 相手ユーザーのカーソル位置をCanvas上に十字カーソルとして表示（ユーザー別カラー）
...
│   ├── PitchCanvas.tsx       # ピアノロール描画（編集モード・リモートカーソル）
...
# 共同編集モード（Next.js + Socket.ioサーバーを同時起動）
npm run dev:collab
```
...
2. ヘッダーの「Collab」をクリック → ルームが自動作成される
3. 「Copy Invite Link」で招待URLをコピー
4. 別のブラウザ/タブで招待URLを開く → 共同編集開始

### 同期される内容

- ターゲットカーブ描画（ピアノロール上の編集）
